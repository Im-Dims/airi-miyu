  ...
  run: async (m, { q, text, conn, repl, Func }) => {
    ...
    try {
      ...
      if (m.isGc) {
        ...
        await conn.sendFromAIMessage(m.chat, message, m)
      } else {
        await conn.sendMessage(m.chat, { text: message }, { quoted: m })
      }
    } catch (e) {
      console.log(e)
      return repl('Maaf, Miyu lagi error nih :(')
    }
  }
}
